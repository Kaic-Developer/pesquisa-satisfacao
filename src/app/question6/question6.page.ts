...
export class Question6Page implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (currentResponse && currentResponse.resposta6) {
      this.resposta = currentResponse.resposta6;
      this.updateGifSrc();
    }
  }
  ...
  async proxima(): Promise<void> {
    if (this.resposta) {
      const currentResponse = await this.dataService.get('currentResponse') || {};
      currentResponse.resposta6 = this.resposta;
      await this.dataService.set('currentResponse', currentResponse);

      this.navCtrl.navigateForward('/question7'); // Ou qualquer página de conclusão ou próxima
    } else {
  ...
  voltar(): void {
    this.navCtrl.navigateBack('/question5');
  }
}
