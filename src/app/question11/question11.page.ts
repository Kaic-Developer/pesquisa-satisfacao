...
export class Question11Page implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (currentResponse && currentResponse.resposta11) {
      this.resposta = currentResponse.resposta11;
      this.updateGifSrc();
    }
  }
  ...
  async proxima(): Promise<void> {
    if (this.resposta) {
      const currentResponse = await this.dataService.get('currentResponse') || {};
      currentResponse.resposta11 = this.resposta;
      await this.dataService.set('currentResponse', currentResponse);

      this.navCtrl.navigateForward('/question12');
    } else {
  ...
  voltar(): void {
    this.navCtrl.navigateBack('/question10');
  }
}
