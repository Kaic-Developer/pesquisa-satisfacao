...
export class Question3Page implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (currentResponse && currentResponse.resposta3) {
      this.resposta = currentResponse.resposta3;
      this.updateGifSrc();
    }
  }
  ...
  async proxima(): Promise<void> {
    if (this.resposta) {
      const currentResponse = await this.dataService.get('currentResponse') || {};
      currentResponse.resposta3 = this.resposta;
      await this.dataService.set('currentResponse', currentResponse);

      this.navCtrl.navigateForward('/question4');
    } else {
  ...
  voltar(): void {
    this.navCtrl.navigateBack('/question2');
  }
}
