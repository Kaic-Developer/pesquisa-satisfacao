...
export class Question2Page implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (currentResponse && currentResponse.resposta2) {
      this.resposta = currentResponse.resposta2;
      this.updateGifSrc();
    }
  }
  ...
  async proxima(): Promise<void> {
    if (this.resposta) {
      const currentResponse = await this.dataService.get('currentResponse') || {};
      currentResponse.resposta2 = this.resposta;
      await this.dataService.set('currentResponse', currentResponse);

      this.navCtrl.navigateForward('/question3');
    } else {
  ...
  voltar(): void {
    this.navCtrl.navigateBack('/question1');
  }
}
